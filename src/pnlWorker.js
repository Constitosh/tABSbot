// src/pnlWorker.js
// Accurate wallet PnL on Abstract: normal ETH, internal ETH, WETH legs, router/proxy settlements.
// Totals for header are derived ONLY from token trades (sum of buy ETH and sell ETH across tokens).
...
// Known routers/forwarders (extendable)
const KNOWN_ROUTERS = new Set([
  '0x0d6848e39114abe69054407452b8aab82f8a44ba', // Moonshot router
  '0x1c4ae91dfa56e49fca849ede553759e1f5f04d9f', // TG buy/sell proxy (forwarder)
].map(x => x.toLowerCase()));

// ---------- Etherscan client with throttle (≤ 5 rps) ----------
...
async function throttle() {
  await (chain = chain.then(async () => {
    const gap = Math.max(0, last + Math.ceil(1000/ES_RPS) - Date.now());
...
async function getUsdQuote(ca) {
  try {
    ...
    return { priceUsd: Number(item?.priceUsd || 0) || 0 };
...
async function getQuotes(ca) {
  const [u,w] = await Promise.all([getUsdQuote(ca), getWethQuote(ca)]);
  return { priceUsd: u.priceUsd, priceWeth: w.priceWeth };
}

// ---------- History pulls ----------
async function getWalletERC20Txs(addr, { fromTs=0 }={}) {
  ...
  while (true) {
    const res = await esGET({
      ...
      startblock:0, endblock:999999999, page, offset:1000, sort:'asc'
    });
...
async function getWalletNormalTxs(addr, { fromTs=0 }={}) {
  const wallet = String(addr).toLowerCase();
  const out = [];
  let page = 1;
  while (true) {
    const res = await esGET({
      ...
      startblock:0, endblock:999999999, page, offset:10000, sort:'asc'
    });
    if (!Array.isArray(res) || res.length===0) break;
    for (const r of res) if (Number(r.timeStamp||0) >= fromTs) out.push(r);
...
async function getWalletInternalTxs(addr, { fromTs=0 }={}) {
  const wallet = String(addr).toLowerCase();
  const out = [];
  let page = 1;
  while (true) {
    const res = await esGET({
      module:'account', action:'txlistinternal', address: wallet,
      startblock:0, endblock:999999999, page, offset:10000, sort:'asc'
    });
    if (!Array.isArray(res) || res.length===0) break;
    for (const r of res) if (Number(r.timeStamp||0) >= fromTs) out.push(r);
...
async function getEthBalance(addr) {
  try {
    const r = await esGET({ module:'account', action:'balance', address: addr, tag:'latest' });
...
async function computePnL(wallet, { sinceTs=0 }) {
  ...
  const [erc20, normal, internals, ethBalWeiStr] = await Promise.all([
  ...
  const blockEthNet     = new Map(); // block -> net +in/-out
  const blockWethNet    = new Map();

  // native ETH from normal
  ...
  for (const [token, txs] of tokenTxsByToken.entries()) {
    ...
    let qty = 0n;                // token units held
    let costWeth = 0n;           // cost basis of remaining (wei)
    let realizedWeth = 0n;       // realized PnL in wei
    let buysUnits = 0n, sellsUnits = 0n;

    let totalBuysEthWei = 0n;    // ETH spent to buy this token
    let totalSellsEthWei = 0n;   // ETH received from selling this token
    ...
    for (const r of txs) {
      ...
      // BUY: token comes in; either pay ETH/WETH in this tx or bonding (from === token)
      if (to === wallet && (paidWei > 0n || from === token)) {
        qty += amt; buysUnits += amt;
        costWeth += paidWei;
        totalBuysEthWei += paidWei;
        continue;
      }

      // SELL: token leaves wallet and we receive ETH/WETH in this tx
      if (from === wallet && recvWei > 0n) {
        sellsUnits += amt;
        totalSellsEthWei += recvWei;

        const avgCostWeiPerUnit = qty>0n ? (costWeth * 1_000_000_000_000_000_000n) / qty : 0n;
        const useAmt = amt>qty ? qty : amt;
        const costSold = (avgCostWeiPerUnit * useAmt) / 1_000_000_000_000_000_000n;
      ...
      // Gift/out without proceeds: reduce inventory + basis proportionally
      if (from === wallet && recvWei === 0n) {
        if (qty > 0n) {
          ...
          qty -= useAmt;
          costWeth = costWeth>dec ? (costWeth - dec) : 0n;
          sellsUnits += useAmt;
        }
        continue;
      }

      // Airdrops: token in, no ETH leg, not from token itself
      if (to === wallet && paidWei === 0n && from !== token) {
    ...
    // Dust filter: keep closed anyway; hide open < 5 tokens (except ETH/WETH – which we already exclude from lists)
    const minUnits = 5n * (10n ** BigInt(decimals));
    const open = qty > 0n;
    const openDust = open && qty < minUnits;
  ...
  // Derived
  const openPositions = perToken.filter(t => t.isOpen && t.usdValueRemaining >= 1 && !['ETH','WETH'].includes(String(t.symbol).toUpperCase()));
  const airdropsFlat  = perToken
    .filter(t => (t.airdrops?.count || 0) > 0 && !['ETH','WETH'].includes(String(t.symbol).toUpperCase()))
    .map(t => ({ token: t.token, symbol: t.symbol, decimals: t.decimals, units: t.airdrops.units, estUsd: t.airdrops.estUsd }));

  // Realized leaders (closed / tiny-dust treated as closed). We rely on realizedWeth sign only.
  ...
  const best  = realizedOnly.filter(x => x.realizedWeth>0).sort((a,b)=> b.realizedWeth-a.realizedWeth).slice(0,15);
  const worst = realizedOnly.filter(x => x.realizedWeth<0).sort((a,b)=> a.realizedWeth-b.realizedWeth).slice(0,15);

  return {
    wallet,
    sinceTs,
    totals: {
      ethBalance: ethBalanceFloat,       // wallet ETH bal (for header)
      ethInFloat:  sumSell,              // trade-only ETH in
      ethOutFloat: sumBuy,               // trade-only ETH out
